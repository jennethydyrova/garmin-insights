...
async def get_step_goal_percent(
    ...
    return build_metric_response(
        ...
        unit="%",
        description=f"""Step goal progress: {step_goal_percent:.2f}% ({data.total_steps:,}
                    / {data.daily_step_goal:,} steps)""",
    )
...
async def get_sedentary_minutes(
    ...
    sedentary_ratio = (
        convert_seconds_to_minutes(data.sedentary_seconds)
    ) / minutes_since_midnight

    return build_metric_response(
        value=sedentary_ratio,
        unit="ratio",
        description=f"""Sedentary time ratio: {sedentary_ratio:.2f} ({data.sedentary_seconds // 60}
                    min sedentary / {minutes_since_midnight} min elapsed)""",
    )
...
async def get_active_minutes_percent(
    ...
    return build_metric_response(
        ...
        unit="%",
        description=f"""Active time percentage: {active_percent:.2f}% ({data.active_seconds // 60}
                    min active / {minutes_since_midnight} min elapsed)""",
    )
