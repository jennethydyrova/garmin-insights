...
from ..utils import Metric, SleepData, convert_seconds_to_hours, convert_to_percentage, get_garmin_data_dependency
...
def analyze_stage_composition(data: SleepData) -> str:
    ...
    def analyze_stage(stage_name: str, percent: float, min_val: int, max_val: int) -> str:
        ...
        if gap_to_min < 0:
            gap_minutes = (abs(gap_to_min) / 100) * (convert_seconds_to_minutes(data.sleep_time))
...
async def get_sleep_fragmentation_index(
    ...
    if data.awakenings is not None:
        ...
        fragmentation = (convert_seconds_to_hours(data.awake_duration)) / total_sleep_hours
